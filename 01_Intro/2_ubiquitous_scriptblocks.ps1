...
# have FUN:
[ValidateScript({if((Test-Path -Path $_)) {[Console]::Beep(1200, 300);$true}else{[Console]::Beep(400, 600);$false}})]$path = 'c:\windows'
$path = 'c:\windows'
$path = 'c:\wrongPath'
...
# PS7:
1..10 | Foreach-Object -Parallel { "processing $_"; Start-Sleep -Seconds 1; 'done' } -ThrottleLimit 10
# WPS:
#requires -Module PSParallel
1..10 | Invoke-Parallel { "processing $_"; Start-Sleep -Seconds 1; 'done' } -ThrottleLimit 10
